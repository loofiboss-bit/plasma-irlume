...
class KFaceAuthKcmTest final : public QObject
{
    Q_OBJECT
...
void KFaceAuthKcmTest::constructionKeepsEventLoopResponsive()
{
    BackendState state;
    QElapsedTimer elapsed;
    elapsed.start();
    KFaceAuthKcm kcm(nullptr, KPluginMetaData{}, std::make_unique<PendingBackend>(&state));
...
void KFaceAuthKcmTest::destructionCancelsActiveRefresh()
{
    BackendState state;
    auto kcm = std::make_unique<KFaceAuthKcm>(nullptr, KPluginMetaData{}, std::make_unique<PendingBackend>(&state));
...
QTEST_MAIN(KFaceAuthKcmTest)

#include "test_kfaceauthkcm.moc"
